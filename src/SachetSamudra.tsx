...
<!-- DRAFT generated by OceanWatch. Not issued. Publishing requires NDMA SACHET authorisation. -->
<alert xmlns="urn:oasis:names:tc:emergency:cap:1.2">
  <identifier>${escapeXml(a.id)}</identifier>
  <sender>oceanwatch</sender>
...
export default function SachetSamudra() {
  ...
  useEffect(() => {
    ...
    setBody(
      `Oil pollution has been reported ${offshore > 2 ? 'offshore' : 'along the coast'}. Location: ${activeCase.subRegion}. ` +
  ...
  const sightingColumns: Column<SightingReport>[] = [
    {
      ...
      render: (s) => (
        ...
        <div className="text-[0.65625rem] text-gray-400">{fmt.ago(s.receivedAt, now)}</div></div>
      ),
    },
    {
      ...
      render: (s) => (
        <div className="min-w-0">
          ...
          <div className="text-[0.6875rem] text-gray-500 truncate" title={s.description}>{s.description}</div>
        </div>
      ),
    },
      ...
      render: (s) => (
        <div className="flex gap-1">
          {s.verified
            ...
            : <Button size="sm" disabled={!editable} title={editable ? undefined : READ_ONLY_HINT} onClick={(e) => { e.stopPropagation(); verifySighting(s.id); }}>Verify</Button>}
          {!s.linkedCaseId && <Button size="sm" disabled={!editable} title={editable ? undefined : READ_ONLY_HINT} onClick={(e) => { e.stopPropagation(); setLinkModal(s.id); }} icon={<Link2 className="w-3 h-3" />}>Link</Button>}
        </div>
      ),
    },
  ];
    ...
    <main className="flex-1 min-h-0 flex flex-col lg:flex-row overflow-y-auto lg:overflow-hidden">
      <aside className="w-full lg:w-[320px] xl:w-[380px] bg-white border-b lg:border-b-0 lg:border-r border-gray-200 flex flex-col flex-shrink-0 max-h-[46vh] lg:max-h-none">
        <div className="px-3 py-2 border-b border-gray-200 bg-gray-50">
          ...
          <p className="text-[0.6875rem] text-gray-500 mt-0.5">Draft CAP alerts and field observation reports</p>
        </div>
        ...
        {tab === 'compose' && (
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div>
              <p className="text-[0.6875rem] font-bold text-gray-600 uppercase mb-1">Channels requested</p>
            ...
            <div>
              <p className="text-[0.6875rem] font-bold text-gray-600 uppercase mb-1 flex items-center gap-1.5"><Languages className="w-3 h-3" /> Languages</p>
              <div className="flex flex-wrap gap-1">
                {LANGUAGES.map((l) => (
                  <button key={l} onClick={() => setLanguages(languages.includes(l) ? languages.filter((x) => x !== l) : [...languages, l])}
                    className={`text-[0.6875rem] px-1.5 py-0.5 rounded border font-semibold ${
            ...
            <div>
              <p className="text-[0.6875rem] font-bold text-gray-600 uppercase mb-1">Coastal districts</p>
              <div className="flex flex-wrap gap-1">
              ...
              {districts.length === 0 && <p className="text-[0.6875rem] text-amber-700 mt-1">Select at least one district.</p>}
            </div>
            ...
            <Button variant="primary" className="w-full justify-center" disabled={!canDraft || !editable} title={editable ? undefined : READ_ONLY_HINT} onClick={doDraft} icon={<Send className="w-3.5 h-3.5" />}>
              Draft CAP alert
            </Button>

            <InfoBanner tone="amber" icon={<Radio className="w-3.5 h-3.5" />}>
              Drafts a CAP 1.2 message for download. Sending through SACHET needs NDMA authorisation, so delivery figures are not shown.
            </InfoBanner>
          </div>
        )}
            ...
            {world.alerts.length === 0 ? (
              ...
              <div key={a.id} className="px-3 py-2.5 border-b border-gray-100">
                <div className="flex justify-between items-start gap-2 mb-1">
                  <div className="min-w-0">
                    <p className="text-[0.75rem] font-bold text-gray-900">{a.headline}</p>
                    <p className="text-[0.6875rem] text-gray-500 font-mono">{a.id} · {a.caseId}</p>
                  </div>
                ...
                <p className="text-[0.6875rem] text-gray-600 leading-normal line-clamp-2">{a.body}</p>
      ...
      <section className="flex-1 min-w-0 min-h-[72vh] lg:min-h-0 flex flex-col flex-shrink-0 lg:flex-shrink">
        ...
        <div className="h-[250px] border-t border-gray-200 bg-white flex-shrink-0 flex flex-col">
          <div className="px-3 py-1.5 border-b border-gray-200 bg-gray-50 flex items-center justify-between gap-3">
            <span className="text-[0.75rem] font-bold text-gray-700 flex items-center gap-1.5">
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" disabled={!editable} title={editable ? undefined : READ_ONLY_HINT} onClick={() => setReportOpen(true)} icon={<Plus className="w-3 h-3" />}>Log report</Button>
            </div>
          </div>
      ...
      <div className="space-y-4">
        <div className="bg-gray-50 border border-gray-200 rounded p-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[0.75rem] font-bold text-gray-900">{s.reporter} — {s.district}</span>
            <Badge tone={s.severity === 'Heavy Oil' ? 'red' : 'amber'}>{s.severity}</Badge>
          </div>
          <p className="text-[0.75rem] text-gray-700 leading-relaxed italic">"{s.description}"</p>
          <p className="text-[0.6875rem] text-gray-500 mt-1.5 font-mono">
            ...
            {ranked.slice(0, 5).map(({ c, d }) => (
              <button key={c.id} onClick={() => setPick(c.id)}
                ...
                <div className="flex justify-between items-center gap-2">
                  <div className="min-w-0">
                    <span className="text-[0.75rem] font-bold text-gray-900">{c.title}</span>
                    <span className="text-[0.6875rem] text-gray-600 ml-2 truncate">{c.subRegion}</span>
                  </div>
                  <span className="text-[0.6875rem] font-mono text-gray-500 flex-shrink-0">{d.toFixed(0)} km</span>
                </div>
                <p className="text-[0.6875rem] text-gray-400 mt-0.5">Incident {fmt.precise(c.incidentTime, c.facts.incident.timePrecision)}</p>
              </button>
            ))}
          </div>
        </Field>
