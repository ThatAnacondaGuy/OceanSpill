...
export function StatCard({
  ...
  return (
    <button
      onClick={onClick}
      disabled={!onClick}
      className={`bg-white rounded-lg p-3.5 sm:p-4 shadow-sm border border-gray-200 flex flex-col sm:flex-row items-start sm:items-center gap-2.5 sm:gap-3.5 text-left flex-1 min-w-[140px] ${
        ...
        <thead className={`text-[11px] text-gray-600 bg-gray-50 border-b border-gray-200 ${stickyHeader ? 'sticky top-0 z-10' : ''}`}>
          <tr>
            {columns.map((c) => (
              <th
                key={c.key}
                // Free-width columns (names, titles) keep a readable minimum instead of collapsing.
                style={c.width ? { width: c.width } : { minWidth: dense ? 150 : 220 }}
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <input
        ...
        className="w-full pl-8 pr-7 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 bg-white"
      />
      {value && (
...
export function Toggle({ checked, onChange, label, count }: { checked: boolean; onChange: (v: boolean) => void; label: ReactNode; count?: number }) {
  return (
    <label className="flex items-center gap-2.5 cursor-pointer text-xs py-1.5 group select-none">
      <span
...
export function TextInput({
  ...
  return (
    <input
      ...
      className={`w-full px-2.5 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 ${mono ? 'font-mono' : ''} ${className}`}
    />
  );
}
...
export function TextArea({
  ...
  return (
    <textarea
      ...
      className={`w-full px-2.5 py-2 text-xs border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 resize-y ${className}`}
    />
  );
}

export function Tabs({
  tabs, active, onChange, className = '', fill,
}: { tabs: { id: string; label: ReactNode; count?: number }[]; active: string; onChange: (id: string) => void; className?: string; fill?: boolean }) {
  return (
...
export function TimeScrubber({
  ...
  return (
    <div className="bg-white border-t border-gray-200 px-3.5 py-2.5 flex flex-wrap sm:flex-nowrap items-center gap-x-3 gap-y-1.5">
    ...
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height }}>
      {ticks.map((t, i) => (
        <g key={i}>
          ...
          <text x={padL - 4} y={yAt(t) + 3} textAnchor="end" fontSize="9" fill="#6b7280" fontFamily="ui-monospace, monospace">
      ...
      {markers.map((m, i) => (
        <g key={i}>
          ...
          <text x={xAt(m.x) + 3} y={padT + 8} fontSize="9" fill={m.color} fontWeight="700">{m.label}</text>
        </g>
      ))}
      {series.map((s) => {
      ...
      {xLabels && xLabels.map((l, i) => {
        ...
        return <text key={i} x={xAt(idx)} y={H - 5} textAnchor="middle" fontSize="9" fill="#6b7280">{l}</text>;
      })}
    </svg>
  );
}
    ...
    <div className={`grid gap-x-5 gap-y-2 text-[12px]`} style={{ gridTemplateColumns: `repeat(${cols}, minmax(0,1fr))` }}>
      {items.map(([k, v], i) => (
        ...
        <div key={i} className="flex justify-between gap-3 border-b border-gray-100 pb-1.5 min-w-0"
          style={cols > 1 && typeof v === 'string' && v.length > 20 ? { gridColumn: '1 / -1' } : undefined}>
          <span className="text-gray-500 min-w-0 [&>span]:flex-wrap">{k}</span>
          <span className="font-semibold text-gray-900 text-right flex-shrink-0 max-w-[70%] [overflow-wrap:anywhere]">{v}</span>
        </div>
      ))}
    </div>
  );
