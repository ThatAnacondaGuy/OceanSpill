...
import {
  ...
  EmptyState, StatCard, triggerDownload,
} from './components/ui';
...
export default function NcscmEcological() {
  ...
  const exportBriefing = () => {
    ...
    const lines = [
      ...
      `Detected       : ${fmt.utc(activeCase.detection.acquiredAt)} (${activeCase.detection.sensor})`,
      `Confidence     : ${fmt.pct(analysis.assessment.confidence)} — ${analysis.assessment.verdict}`,
      `Oil type       : ${activeCase.oilType}`,
      `Volume         : ${activeCase.estimatedVolumeM3} m³ estimated`,
      ...
      '',
      `Generated by OceanWatch AI · National Marine Pollution Intelligence System`,
    ];
    ...
    <main className="flex-1 min-h-0 flex overflow-hidden">
      <aside className="w-[340px] bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
        <div className="px-3 py-2 border-b border-gray-200 bg-gray-50">
          ...
          <p className="text-[10px] text-gray-500 mt-0.5">NCSCM designated areas · {ECOLOGICAL_AREAS.length} sites</p>
        </div>

        <div className="p-2 border-b border-gray-200">
          <Select label="Active case" value={activeCase?.id ?? ''} onChange={(v) => setSelectedCaseId(v)}
            options={world.cases.map((c) => ({ value: c.id, label: `${c.id} — ${c.region}` }))} />
        </div>
                ...
                <div className="relative">
                  ...
                  {layersOpen && (
                    <div className="absolute top-full mt-1 left-0 bg-white rounded shadow-xl border border-gray-300 p-2.5 w-52 z-30">
                      ...
                      <Toggle checked={layers.slick} onChange={(v) => setLayers({ ...layers, slick: v })} label="Detected slick" />
