...
/** Shore types, most consequential first, with the colour used beside each. */
const SHORE_TONE: Record<string, string> = {
  mangrove: 'bg-emerald-700',
  marsh: 'bg-teal-600',
  coral: 'bg-rose-500',
  beach: 'bg-amber-400',
  rocky: 'bg-slate-500',
...
export default function NcscmEcological() {
  ...
  // Opened from search with a protected area: select it and show the area register.
  useEffect(() => {
    const s = consumeSection();
    if (s && ECOLOGICAL_AREAS.some((a) => a.id === s)) { setSelectedArea(s); setTab('register'); }
  }, [consumeSection]);
  ...
  // What the shore is made of along this case's coastline, where the pipeline has worked it out.
  const shore = activeCase ? world.shoreTypes.get(activeCase.id) : undefined;
  const mappedBoundaries = world.protectedBoundaries;
  ...
  const priority = useMemo(() => {
    ...
    return analysis.threatenedAreas
      .map((t) => {
        ...
        const level = t.hoursToImpact !== null ? 3 : t.distanceKm < 40 ? 2 : t.distanceKm < 120 ? 1 : 0;
        const action = ['No action — outside the credible impact envelope', 'Monitor. Re-evaluate on the next satellite pass',
          'Pre-position boom and skimmer; place response crew on standby', 'Deploy containment boom at the seaward approach immediately'][level];
        return { ...t, area, score, action, level };
      })
      // Urgency of the recommended action first, then the combined sensitivity and proximity score.
  ...
  const exportBriefing = () => {
    ...
    const lines = [
      ...
      'This is a retrospective analysis of a recorded incident, not an operational forecast.',
      '',
      `Generated by OceanSpill — Oil Spill Detection & Vessel Attribution System`,
    ];
  ...
  // Without a selected area, frame the active case: slick, forecast envelope and the nearest protected area.
  const caseFrame = useMemo(() => {
    if (!activeCase || !analysis) return undefined;
    const pts = [...activeCase.detection.polygon.ring, ...analysis.forecast.horizons.map((h) => h.centroid)];
    const nearest = priority[0] ? ECOLOGICAL_AREAS.find((a) => a.id === priority[0].id) : undefined;
    if (nearest) pts.push(...nearest.ring);
    return pts;
  }, [activeCase, analysis, priority]);
    ...
    <main className="flex-1 min-h-0 flex flex-col lg:flex-row overflow-y-auto lg:overflow-hidden">
      <aside className="w-full lg:w-[290px] xl:w-[340px] bg-white border-b lg:border-b-0 lg:border-r border-gray-200 flex flex-col flex-shrink-0 max-h-[46vh] lg:max-h-none">
        <div className="px-3 py-2 border-b border-gray-200 bg-gray-50">
          ...
          <p className="text-[0.6875rem] text-gray-500 mt-0.5 flex items-center gap-1.5">{ECOLOGICAL_AREAS.length} sites <ProvenanceBadge p={mappedBoundaries ? 'real' : 'modelled'} /></p>
          <p className="text-[0.6875rem] text-gray-400 mt-0.5 leading-normal">
            {mappedBoundaries
              ? `${mappedBoundaries} of ${ECOLOGICAL_AREAS.length} outlines come from OpenStreetMap; the rest are drawn from published maps. Official NCSCM shapefiles are not published for download.`
              : 'Official NCSCM shapefiles pending; outlines are hand-drawn from published maps.'}
          </p>
        </div>
              ...
              <>
                {shore && (
                  <div className="p-3 border-b border-gray-200">
                    <p className="text-[0.6875rem] font-bold text-gray-600 uppercase mb-1.5">What the shore is made of</p>
                    <div className="space-y-1">
                      {Object.entries(shore.summary.kilometres)
                        .filter(([kind]) => kind !== 'unclassified')
                        .slice(0, 5)
                        .map(([kind, km]) => (
                          <div key={kind} className="flex items-center gap-2" title={shore.descriptions[kind]}>
                            <span className={`w-2 h-2 rounded-full flex-shrink-0 ${SHORE_TONE[kind] ?? 'bg-gray-400'}`} />
                            <span className="text-[0.75rem] text-gray-800 capitalize flex-1">{kind}</span>
                            <span className="text-[0.75rem] font-mono text-gray-600">{fmt.num(Math.round(km))} km</span>
                          </div>
                        ))}
                    </div>
                    <p className="text-[0.6875rem] text-gray-500 mt-1.5 leading-normal">
                      {Math.round(shore.summary.classifiedFraction * 100)}% of {fmt.num(Math.round(shore.summary.totalKm))} km
                      of coastline in this case's box is mapped in OpenStreetMap; the rest is not classified.
                    </p>
                  </div>
                )}
                <div className="p-3 border-b border-gray-200">
                  ...
                  <div key={p.id} onClick={() => setSelectedArea(p.id === selectedArea ? null : p.id)}
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[0.75rem] font-bold text-gray-900 leading-snug">{p.name}</p>
                        <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                          <Badge tone={p.sensitivity >= 5 ? 'red' : p.sensitivity >= 4 ? 'amber' : 'green'}>S{p.sensitivity}/5</Badge>
                          <span className="text-[0.6875rem] text-gray-500">{p.category}</span>
                        </div>
                        <div className="flex gap-3 mt-1 text-[0.6875rem]">
                        ...
                        <p className={`text-[0.6875rem] mt-1 leading-normal ${p.hoursToImpact !== null ? 'text-red-700 font-semibold' : 'text-gray-600'}`}>
                          {p.action}
                        </p>
                        {p.id === selectedArea && (
                          <div className="mt-2 space-y-2">
                            <p className="text-[0.6875rem] text-gray-600 leading-normal">{p.area.notes}</p>
                ...
                <button key={a.id} onClick={() => setSelectedArea(a.id === selectedArea ? null : a.id)}
                  ...
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0">
                      <p className="text-[0.75rem] font-bold text-gray-900 leading-snug">{a.name}</p>
                      <p className="text-[0.6875rem] text-gray-500">{a.category} · {a.state}</p>
                    </div>
                    <Badge tone={a.sensitivity >= 5 ? 'red' : a.sensitivity >= 4 ? 'amber' : 'green'}>S{a.sensitivity}</Badge>
                  </div>
                  {a.id === selectedArea && (
                    <div className="mt-1.5">
                      <p className="text-[0.6875rem] text-gray-600 leading-normal">{a.notes}</p>
      ...
      <section className="flex-1 min-w-0 min-h-[72vh] lg:min-h-0 flex flex-col flex-shrink-0 lg:flex-shrink">
        <div className="bg-white border-b border-gray-200 px-3 py-2 flex items-center gap-3 flex-wrap">
          ...
          <div className="ml-auto flex gap-2">
            ...
            {activeCase && (
              <Button size="sm" variant="primary" disabled={!canEdit('NCSCM Ecological') || access('SACHET / SAMUDRA') === 'none'}
                title={!canEdit('NCSCM Ecological') ? READ_ONLY_HINT : access('SACHET / SAMUDRA') === 'none' ? 'Your role cannot open community alerting' : undefined}
                onClick={() => navigate({ tab: 'SACHET / SAMUDRA', caseId: activeCase.id })} icon={<Anchor className="w-3 h-3" />}>
                Issue advisory
              </Button>
            )}
          </div>
        ...
        <div className="flex-1 min-h-0 relative">
          <MapView
            ...
            fitTo={selected ? selected.ring : caseFrame} fitKey={selectedArea ?? `case-${activeCase?.id ?? 'none'}`}
            overlay={
